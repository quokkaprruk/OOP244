...
namespace sdds {
	...
	//public:
		/*
	Publication::Publication() :m_title(nullptr),m_shelfId(),m_date() {
		//strCpy(m_shelfId, '\0'); => wrong
		m_membership = 0; 
		m_libRef = -1;
	}; 
		*/
	...
	std::ostream& Publication::write(std::ostream& os) const {
		...
		if (conIO(os)) {
			...
			* //const int TABLE_WIDTH = 70;
			for (int i = 0, j = 1; i < TABLE_WIDTH; i++) {
				if ((i + 1) % 10 == 0) {
					os << j;
					j++;
				}
			...
			*/
			...
			//os  << left << setw(6) << setfill(' ') << m_membership;
			os << " | " << m_date << " |";// << endl;
		}
		else {
			os << type() << "\t\t"; //print 'P'
	...
	std::istream& Publication::read(std::istream& istr) {
		//temp variable
		char temp_title[SDDS_TITLE_WIDTH + 1] = { '\0' }; //getline is used for char[] not char*
		...
		if (conIO(istr)) {
			...
			istr.getline(temp_title, SDDS_TITLE_WIDTH+1, '\t'); //getline is used for char[] not char*
	...
	bool Publication::conIO(std::ios& iosObj)const {
		//helps determine if a given input/output object is associated with 
		//console input or output operations or if it refers to some other 
