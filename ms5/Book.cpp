...
//public:
	...
	Book::Book(const Book & src):Publication(src) { ////responsible for base and also derived class
		if (this != &src) {
			if (b_author != nullptr) {
				delete[] b_author;
				b_author = nullptr;
	...
	Book& Book::operator=(const Book& src) { // dont forget const!!!
		if (this != &src) {
			Publication::operator=(src);//responsible for base and also derived class
			if (b_author != nullptr) {
				delete[] b_author;
				b_author = nullptr;
			}
