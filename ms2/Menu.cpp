...
namespace sdds {
	...
	//class Menu{
		//MenuItem m_Title; hold tilte of the Menu
		//MenuItem* m_items[MAX_MENU_ITEMS]; Initialize this array of pointers to nullptrs.
		//int menuItemCount;
	//---------------------------
	...
	Menu::Menu(const char* newtitle) {
		if (newtitle) {
			m_title = new char[strlen(newtitle) + 1]; why can't use =
	...
	Menu& Menu::operator<<(const char* menuitemContent) {
		...
		if (menuItemCount < MAX_MENU_ITEMS) {
			...
			/*
			 ...
			 m_items[menuItemCount] = newMenuItem; //
			*/
	...
	//why return different data type with the same variable? 
